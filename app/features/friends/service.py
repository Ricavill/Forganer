...
from app.features.friends.schemas import InterestedFriendOut
...
def send_request(db: Session, from_user_id: int, to_user_id: int) -> FriendInvitation:
...
def list_incoming_requests(db: Session, user_id: int) -> list[FriendInvitation]:
    result = db.execute(
        select(FriendInvitation).where(
...
def accept_request(db: Session, invitation_id: int, user_id: int) -> FriendInvitation:
...
def reject_request(db: Session, invitation_id: int, user_id: int) -> FriendInvitation:
    invitation = _get_pending_invitation_for_recipient(db, invitation_id, user_id)
